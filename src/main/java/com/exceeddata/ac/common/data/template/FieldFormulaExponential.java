...
public class FieldFormulaExponential implements FieldFormula {
    ...
    public FieldFormulaExponential(
    ...
    public TypeData calculate(final TypeData data) throws EngineException {
        ...
        if (p3zero) {
            return DoubleData.nonNullValueOf(Math.log(value.subtract(p6).multiply(p5).subtract(p2).divide(p0).doubleValue()) / p1d);
        }
        if (p0zero) {
            ...
            return DoubleData.nonNullValueOf(Math.log(p2.divide(denom).subtract(p5).divide(p3).doubleValue()) / p4d);
        }
        return DoubleData.NULL;
    }
